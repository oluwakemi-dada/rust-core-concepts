...
fn main() {
    let mut latte = Coffee::new("Latte", Milk::Whole, 32);
    println!("{:?}", latte);
    latte.consume();
    println!("{:?}", latte);

    let cappuccino = Coffee::new(String::from("Cappuccino"), Milk::Almond, 25);
    println!("{:?}", cappuccino.get_data());

    let pepsi = Soda::new(650, 2.50, String::from("Cherry"));
    println!("{}", pepsi);
